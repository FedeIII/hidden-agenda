...
**`NEXT TURN` beats while it waits, and the beat may not move it.** A turn that has ended looks exactly like one that has not until somebody notices the button has lit, so `Button` takes a `$beat` prop and the strip sets it on the same predicate as `active`. **Two** beats and then a rest — a double knock rather than a single tap, because one soft pulse every couple of seconds was easy to sit through — and it stops on `:hover`, `:active` and `:focus-visible`, because it was only ever asking for one. Each direction says *ready* from two tokens, `--ha-control-beat-wash` and `--ha-control-beat`: a second impression of Dossier's own stamp outline bleeding into the stock, Blueprint's chalk keyline lighting to the ferro red a drawing calls things out in, Vault's brass catching the light.

- **Loudness goes into the edge, never into the fill, and that is a legibility rule rather than a taste.** The pseudo-element paints *over* the button's own text, so every point of wash alpha costs a point of contrast on the word `NEXT TURN` — and in Dossier the beat and the control ink are the same red, so a wash past about a quarter, or a bleed wide enough to reach the middle of a 30px control, leaves a solid red block with the label invisible inside it. Both directions that could swallow their label hold the wash where it was and spend the extra on the ring and the bevel. Blueprint can afford more of both: its keyline is chalk and its ink is not the annotation colour.
