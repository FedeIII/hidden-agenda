...
| Board marks | — | coordinate ticks and a dimension line reading `7 CELLS · 37 PLAYABLE` | — |
| The piece in hand | a typed slip clipped to it on a leader of dots, `SUBJECT 0-A1 · AGENT, TEAM 0` | called out on a leader line, the number in a disc, no word in front of the id | an engraved plate bezelled in brass, the number on a brass tab, `ITEM 0-A1 · …` |
...
| What is around the board | the rest of the desk: an open typed file part-blacked-out and stamped `EYES ONLY`, a fountain pen and a pencil, a cold coffee and its rings, a rotary telephone with the cord running off | the watermark `PROPERTY OF ██████ INDUSTRIES` repeating behind everything | the lining of the case: die-cut foam with spare pieces still seated in it, `PROPERTY OF ██████ · FIELD SET 04 OF 12 · DO NOT REMOVE FROM ROOM` stencilled up both inside walls, and the maker's plate riveted on |
...
**What is around the board is `theme/ground.js`, and it is four lists, not one.** A direction's ground is a stack of background layers and each layer needs a size, a place to sit and an answer about tiling — so `--ha-ground-wash` has `--ha-ground-size`, `--ha-ground-position` and `--ha-ground-repeat` beside it, consumed by `html` in `globalStyle.js` and by `ScreenStyled` (the alignment-card shell, which is the same ground so the cards are dealt on the desk). **The four lists must stay the same length**: CSS cycles a short list rather than complaining, so a missing entry hands layer four layer one's position, at layer one's size, tiled — and it looks like a decision. `skin.test.js` counts all four back off the computed style. Sizes are `min(<px>, <vmin>)` so a prop is a prop on a laptop and not the whole screen on a phone, and the attachment is `fixed` so the props are anchored to the corners of the *screen* rather than to a document that may be taller than it.

**Nothing in that file hand-encodes its data URI — `asset()` does it.** That is the same stylis hazard the token file's header describes, and hand-encoding is how it was found the first time: `encodeURIComponent` covers the slashes, angle brackets and hashes, and the two round brackets it deliberately leaves alone are replaced after it. Blueprint's watermark went through it too, so there is one mechanism rather than one mechanism and one precedent.

`BoardMarks` (the marks laid over the board) sits over the board and must keep **`pointer-events: none`**: the ring cells it labels are clickable, because that is how a piece on the border is pointed off the board, and an absolutely positioned label over one would quietly eat that. Its offsets come from the layout the renderer already returned, through the `style` prop — never a styled-components template, for the rule-leak reason above.
...
- **The pieces have names in the catalog** (`play.pieceAgent` … `play.pieceSniper`), because the last-move mark says one. Deliberately *not* the `TYPE_NAMES` table in `tableBoard.jsx`: that one is lettering on the mark each direction lays on the board — a drawing's callout, a typed slip, an engraved plate — and stays English in every language, the way `FIG. 1` and `SECTION A–A` do. The word in front of it is the direction's own and *is* translated, because it is in `SKIN_WORDS` (`--ha-mark-key`).
...
| `domain/pieces/constants.js` | The piece types and ids, the `pieceState` machine's states, the points table — and `MOVES`, the four things a turn can leave behind that the last-move mark names. |
...
DOM ids the tests depend on: `pz-{pieceId}`, `hex-{row}-{cell}`, `board`, `store-{team}`, `claim-{team}`, `controlled-{team}`, `piece-count-{team}-{TYPE}`, plus `next-turn`, `snipe`, `snipe-note`, `snipe-fallen-{pieceId}`, `last-move-{pieceId}`, `accuse`, `reveal`, `reveal-friend`, `reveal-foe`, `start-btn`, `alignments-btn`, `player-name{n}`.

`snipe-note`, `snipe-fallen-{pieceId}` and `last-move-{pieceId}` are all **absent rather than empty** when they have nothing to say — the note renders only in hot-seat, the fallen-sniper mark only while a snipe is armed on a sniper that is off the board, the last-move mark only once a turn has been handed on — so a spec asserting `toHaveCount(0)` is asserting the rule and not a blank string.
...
`src/client/state/index.jsx` holds one `useReducer` behind a `withState` HOC; components read `const [state, dispatch] = useContext(StateContext)`. Slices: `players`, `hasTurnEnded`, `pieces`, `pieceState`, `followMouse`, `snipe`, `snipeWindow`, `piecesPrevState`, `lastMove`, `teamControl`.
...
**`lastMove` is what the last player did, and it is a slice because the board cannot say it.** Everybody moves everybody's pieces here, so a player handed the turn is looking at 32 tokens, one of which has changed, with nothing on screen naming it or saying how. It holds `{ id, position, event, victim? }` — the piece, the cell its move ended on, which of `MOVES` happened there and, for a kill, what it took — and `TableBoard` draws a tag over that cell, in the same hand as the fallen sniper's. Four rules:

- **It is written on `NEXT_TURN` and replaced by the next one.** `pz.getTurnMove` reads the turn that is ending against `piecesPrevState`, asked of the same four fields as `hasBoardChanged` so a move and a turn cannot drift apart. The mover is the piece **alive on both boards and not where it was**, which is what tells it from the pieces that move's kills took off the table, and from a sniper a shot has brought back. A turn that changed nothing — the server passing over a seat that has gone — answers `null` and wipes the mark rather than leaving one two turns old.
- **It lasts until the board changes irreversibly under it, not for the whole of the next turn.** Looking is free: a selection, a hover, arming `SNIPE` and claiming a team can all be taken back and all keep the mark. So can revealing an alignment and accusing somebody — those cost their player something real and cannot be undone, but the pieces stand exactly where the last move left them, and this mark is about the board. `lastMoveReducer` therefore keeps **no list of actions to be kept in step with the game**: it tells a click by what it leaves behind, re-running the very `pz.move` / `pz.toggle` the board is about to take and asking `hasBoardChanged` against `piecesPrevState` — the same reading `hasTurnEndedReducer` uses to decide a turn is a turn. A move, a drop that points a piece somewhere new and a shot all end it, from one predicate.
- **A shot therefore takes the mark with the move it undoes**, without a rule of its own — the rollback is a board change like any other.
- **`MOVES` lives in `domain/pieces/constants.js`**, with the phases and the skin names and for the same reason: the server sends them. `PLACED` / `MOVED` / `KILLED` / `CLAIMED`.

Two things `getTurnMove` decides that are worth stating, because neither can be read off the board afterwards:

- **A kill names what was taken, not what took it.** The killer is standing on the marked cell for anybody to read; the piece that is no longer anywhere is the news. The victim is found on `piecesPrevState` — by the time the mark is written it holds `OUT_POSITION` and cannot say which cell it died on. A CEO's team dies with it *elsewhere*, so the mark names the CEO: that is the piece the move actually reached.
- **Deploying a CEO is *how* a team is claimed**, so `CEO PLACED` and `TEAM CLAIMED` are the same board event. `teamControl[team].controlling` is set by that very placement (`teams.movePieceForControl`) and is what tells them apart — a CEO put down with nobody holding the team is a deployment, one put down with a holder is the claim being completed.

**The two marks want the same cell, and not by accident.** A sniper is fallen because the move that killed it ended where it stood, which is the very cell the last move is marked on — so `TableBoard` drops the last-move mark whenever a fallen sniper is answering for that cell. One tag to a cell, and the one that is also a control keeps it. Both are `HexMark` / `HexTag` in `components.jsx`, one shape drawn from two places for the reason below.
...
- **`DELIVER_MS` is the length and the keyframe percentages are the shape, so the two have to be read together.** 1560ms, at 11% and 75%: a ~170ms settle, a full second held still in the middle, and a ~390ms flight. The hold was 200ms when this landed and was raised because that is long enough to *see* a card and not long enough to read one — the card was gone by the time the eye had finished arriving at it. Changing the duration on its own stretches all three parts, so the stops moved with it; and it is the still frame that should carry any further change, never the flight, because a slower flight only makes the same news take longer to arrive. `dissolve`'s first stop is the same 75%: the stock is under the type for exactly as long as the type is out over the board.
- **It runs on the turn CHANGING, not on the press.** Online, the seat that pressed the button is the one seat that already knew — everybody else was told by a 9px key quietly saying a different name. The server passing over a seat that has gone is announced by the same rule.
- **The first render counts, and `held` starts at `null` so that it does.** The first turn is a turn changing hands: the one arrival at this table nobody was told about used to be the first. `getTurn` throws rather than returning a seat with no name, so `null` can never be a real turn and the comparison cannot miss. A remount is the first render again, so a refresh mid-game announces as well — which is the right answer anyway, and there is nothing in this state that could tell the two apart.
- **That mount announcement is the one thing in this feature the suite had to be told about**, and only in two places. `three.test.js` holds the suite's only PIXEL assertions, and they sample `#hex-3-2` — the middle of the board, directly under a card held at the centre of the screen for a second. Both wait on `settled(page)` first. They wait rather than suppressing the card, because what they are about is what the renderer drew and a spec that turned the app's own chrome off would be measuring a screen no player sees. Nothing else needed a line: every other spec asks the DOM, and the card takes no pointer events — `whatIsUnderTheMiddleOf` resolves through it with `elementFromPoint`, the drag controller resolves a drop the same way, and playwright verifies its own hit target the same way again.
...
- **throughSniperLineOf** — ids of enemy snipers whose line of sight a piece crossed while moving. `SNIPE` highlights snipers that have a target; clicking a highlighted sniper kills what it saw. A whole path is checked rather than its endpoints, so a sniper whose line the path ran *along* is named once per cell: the marks are a set, not a tally.
  ...
  - **The mark is drawn from two places, because the board is drawn two ways.** Flat, `FallenSniper` is a child of the hexagon it marks. Projected, a hexagon is an `opacity: 0` hit target and takes anything inside it down with it, so `TableBoard` lays the mark on the board at the box the renderer gave that cell. `LastMove` has the same pair of paths for the same reason, and both are `pointer-events: none`, for the reason the training coach marks have it: the cell underneath *is* the control.
...
| `three/killMoves.js` | What each piece type does at the moment it kills, the mark it leaves, and when the blow lands — the table, and nothing else. No three.js import, for the same reason `layout.js` has none: the unit project reads it. |
...
### Kills, and the journey home

A kill used to be the quietest thing on the table: the victim's token stopped existing and the killer stood on the cell as if it had walked onto an empty one. This is the third pass at fixing that, and the shape of the answer changed twice. v3.18.0 was a nudge per type in a channel of its own — correct, and invisible. v3.19.0 made them gestures with marks, but the gesture still ran *after* the piece had arrived, so an agent crossed the cell at walking pace and then twitched. **Now two of them carry the piece: the gesture IS the arrival.**

`three/killMoves.js` is the table, `token.js#kill` plays it, `token.js#leave` carries the corpse, and `boardScene.js#killsAmong` decides when.

| | Motion | Marks | Time |
| --- | --- | --- | --- |
| Agent | **carried**: charges from where it stood, ~18% *past* the cell it takes, settles onto it | a point out of the nose, a burst where it lands, a streak behind the run | 0.34s |
| Spy | **carried**: holds still, then cuts its way in — three strokes, right/left/right, unwinding as it arrives | a wide blade across the cell, a burst, a streak | 0.16s + 0.36s |
| Sniper | thrown backward over half its own radius, barrel jumps once, long settle forward | a muzzle jet, a big burst, no streak | 0.46s |
| CEO | swells, rim comes up warm | light on its own rim only | 0.50s |

**A CEO cannot kill, so its row has never been seen in a game.** `getCeoPositions` builds its destinations with `getFreeCells`, which stops at the first occupied cell and never includes it, so a CEO is blocked by every piece and can never land on one — `ceo.test.js`'s "is blocked by any piece" asserts exactly that, and the roadmap lists the capture mechanic for the agent, the spy and the sniper only. The row exists so the table is total over `TYPES`, and so there is nothing to add the day the rules change. Do not tune it by eye; there is nothing to look at.

**Carried and non-carried are two different kinds of curve, and confusing them breaks the board.** A carried curve is a *journey*: it starts at exactly 0, which is the cell the piece was standing on, and ends at exactly 1, which is the cell it took. `drivingThrough(back)` is an ease-out-back, so it goes past 1 on the way and comes back — that overshoot is the part of the piece that comes out of the other side, and it is one function rather than two because a piece that accelerated in, stopped dead and then pushed forward again would read as a bump. A non-carried curve is an *amplitude*: it starts and ends at 0 and is peak-normalised, so its `amount` means "how far, at most". A carried curve is deliberately **not** normalised — it has to land on 1, not on its own maximum. `unit/killMoves.test.js` asserts both, separately, and that a carried gesture has covered at least 80% of the ground by the time it strikes.

How the carry works, because it is not obvious: `token.kill` is called in the same pass as the `set` that gave the token its new cell, so `current` is still the cell being left and `wanted` is already the cell being taken — the only moment both are on hand. It records the difference, then **puts `current` on the destination and draws the whole journey as a rig offset it walks back to nothing.** That takes the ordinary travel off the table, so nothing is easing toward the cell behind the gesture's back, and it takes the arc with it — which is right, because a charge is flat and a piece that lofted would be jumping. Two consequences worth knowing: `distanceToGo()` reads 0 for the whole gesture, and the spy's hold is spent at the *origin* because the carry is read off the clock clamped at zero rather than off whether the gesture is playing.

Six more things hold it together:

- **The move is a function of the killer's TYPE and of nothing else** — not the victim, not the distance, not whose turn it is. Learn three gestures once and you know which piece fired without looking away from where you already were.
- **A swing is added to the bearing and always unwinds to nothing.** Facing is a mechanic here, not a look, so a yaw that ended anywhere but zero would leave the piece pointing somewhere the rules disagree with. The test asserts each swing goes both ways round, so a lean cannot pass as a swing.
- **Only the rig moves, never `group.position`.** The group is what `screenOf` projects to tell `flight.js` where a piece was last drawn, so a charge written there would quietly become the point a later drag flies from. Same rule as the overlay boxes, one level down.
- **The marks are three flat additive quads and nothing cleverer** — `blade` along or across the bearing, `burst` opening where the blow lands, `trail` streaming out behind a carried piece. Additive because that is light falling on the board rather than a shape painted over it, which is what makes one mark work on the palest tile and the darkest without being told which it is on. They are rig children, so a blade swung sideways swings with the piece.
- **The streak runs on a different clock from the other two.** It belongs to the run, not the blow: it is up and gone across the approach and dark by the time the point lands. The blade and the burst run from the strike. A burst is sized from its own progress rather than from its brightness — brightness comes up and goes down again, so a burst sized from it would open, close, and read as a heartbeat.
- **The corpse leaves when the blow lands, not when the state changes.** `killMove.strike` is when that is, `token.kill` takes a callback and fires it once, and the board hangs the release on it. A corpse that left on the state change would already be gone by the time the agent got there, and a charge through an empty tile is not a charge. If there is no gesture to wait for — no token, an unknown type, or `prefers-reduced-motion` — the corpse goes at once, which is what the board did before any of this existed.

**The dead go to the HQ of whoever killed them, and that is the deploy run backwards.** It reuses every part of the machinery a piece coming *out* of a tray already needs: each tray publishes its own centre in page pixels through `flight.js#noteHqPosition`, the board turns that back into a point on its own plane with `toBoard`, and the corpse is simply told to go there — so it lofts across the table on the same travel and the same arc, and `overlay()`/`widen()` are what let it be drawn over the gap and over the tray it is heading for. It keeps its size for the first half of the journey and is gone by the end of it, because a piece that starts shrinking the instant it dies reads as a fault in the renderer. Landing at the killer's HQ is also where the game already puts it: `getKilledPiecesByTeam` tallies a kill under the *killer's* team, so the corpse arrives where its own tally is about to increment.

Two things about the corpse worth keeping:

- **A team whose tray is not on screen has no published position**, and the corpse then goes where it stands rather than nowhere. Nothing in this can be missing in a way that breaks anything.
- **`UNCLAIMED` is a net, not a timeout anybody hits.** Every path that arms a gesture bounds its own wait, but a killer whose token is disposed mid-gesture would otherwise leave a corpse animating — and therefore a frame loop running — for the rest of the game.

**This is 3D only, and deliberately.** Under `?flat` or with no WebGL a kill looks exactly as it always did. Motion is the vocabulary this layer introduced; the flat renderer animates for 200ms at a time and its piece carries the facing in a `transform` that `helpers/get.js` reads back as a matrix, so a gesture written there would be read as a bearing.
