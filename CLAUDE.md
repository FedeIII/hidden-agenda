...
- **It is deliberately not cleared by the new player's own move**, unlike the sniper marks. The question it answers is worth asking right up to the moment you commit your own answer to it.
- **A shot takes it with the move it undoes.** `lastMoveReducer` clears on `pz.isSnipeShot`, or the mark would go on naming a move that no longer happened.
