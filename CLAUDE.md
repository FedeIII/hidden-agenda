...
The index's first door is not a page. **`src/client/phases/lobbyPhase/training/` is a course of ten exercises played on the real board**, reached from `rules-open-training` and living at `#/training/{slug}`. Its own ids: `training-title`, `training-record`, `training-go-{slug}`, `training-step`, `training-verb`, `training-hint`, `training-restart`, `training-finding`, `training-finding-line`, `training-file`, `training-next`, `training-complete`, `training-read-{slug}`, `training-play`, `training-back-to-index`, `training-card-{friend,foe}`, and `training-mark-{targetId}` for a coach mark. `training.test.js` drives it and `unit/training.test.js` walks every exercise through the reducer with no browser.
